...
public class BinaryTree implements Dictionary {
  ...
  public Entry remove(Object key) {
    ...
    if(node==null)
	...
	else if(node==root&&node.leftChild!=null&&node.rightChild==null){
		Entry entry=root.entry;
		root=node.leftChild;
		size--;
		return entry;
	}
	else if(node==root&&node.rightChild!=null&&node.leftChild==null){
		Entry entry=root.entry;
		...
		if(node.leftChild==null&&node.rightChild==null){
			...
			if((node.leftChild==null&&node.rightChild!=null)){
				...
				if(node.rightChild!=null){
				BinaryTreeNode replace=node.rightChild;
				...
				if(atLeast1){
					...
					BinaryTreeNode replace=node.leftChild;
					boolean atLeast1=false;
				...
				node.entry=replace.entry;
				if(atLeast1){
					replace.parent.rightChild=replace.leftChild;
				}else{
